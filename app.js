...
app.get("/loans", checkAuthenticated, async (req, res) => {
  ...
  if (user.role === 'admin') {
    sql = `
      SELECT l.loan_id, l.borrow_date, l.due_date, l.return_date,
  ...
  try {
    ...
    const loans = results.map(loan => ({
      ...loan,
      borrow_date: loan.borrow_date?.toISOString().slice(0, 10),
      due_date: loan.due_date?.toISOString().slice(0, 10),
      return_date: loan.return_date?.toISOString().slice(0, 10) || null
    }));
...
// ✅ GET /loans/:id/edit — show edit form for a loan
app.get("/loans/:id/edit", checkAuthenticated, async (req, res) => {
  const loanId = req.params.id;
  const db = pool.promise();

  try {
    const [results] = await db.query(
      `SELECT * FROM book_loans WHERE loan_id = ?`,
      [loanId]
    );
    ...
    loan.borrow_date = loan.borrow_date?.toISOString().split('T')[0];
    loan.due_date = loan.due_date?.toISOString().split('T')[0];
    loan.return_date = loan.return_date ? loan.return_date.toISOString().split('T')[0] : "";
